...
module TurboDesktop
  ...
  #
  #   TurboDesktop.data_dir                  # => #<Pathname .../Application Support/dev.example.ledger>
  #   TurboDesktop.data_dir(create: true)    # same, but it exists afterwards
  #
    ...
    def data_dir(app_id: nil, create: false, host_os: RbConfig::CONFIG["host_os"], env: ENV)
      dir =
        if (override = presence(env["DESKTOP_DATA_DIR"]))
          ...
          base_dir(host_os: host_os, env: env).join(app_id || TurboDesktop.app_id)
        end

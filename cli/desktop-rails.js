...
// this runs as `npx desktop-rails` argv[1] is the link and import.meta.url is
  ...
  if (!handler) {
    console.error(
      command
        ...
        : "Usage: desktop-rails <command> [options]"
    );
    cmdHelp();
    process.exit(1);
  }
...
function cmdNew(args) {
  ...
  if (!appName) {
    console.error("Usage: desktop-rails new <appname> [--icon <file.png>]");
    console.error("Example: desktop-rails new myapp --icon ./logo.png");
    process.exit(1);
  }
  ...
  // Step 2: Add the desktop-rails gem
  console.log("\nAdding desktop-rails gem...");
  ...
  console.log("\nRunning desktop_rails:install generator...\n");
  try {
    run(join(appDir, "bin", "rails"), ["generate", "desktop_rails:install"], {
      cwd: appDir,
      stdio: "inherit",
    });
  } catch {
    fail(`the desktop_rails:install generator failed in ${appName}/`, finishByHand);
  }
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  desktop/desktop-rails.config.json and run bin/rails server first.
`);
}
...
function cmdInit(args) {
  ...
  console.log("Initializing Desktop Rails in", desktopDir);
  ...
  // Copy icons (the default Desktop Rails icon set)
  ...
  copyFileSync(
    join(PACKAGE_ROOT, "src", "desktop-rails.js"),
    join(desktopDir, "src", "desktop-rails.js")
  );
  copyFileSync(
  ...
  // scaffolded app "desktop-rails", carry a bin pointing at a cli/ directory
  ...
  writeFileSync(
    join(desktopDir, "desktop-rails.config.json"),
...
function cmdDev() {
  console.log("Starting Desktop Rails in development mode...");
  ...
  if (!tauriConf) {
    console.error(
      ...
      'Hint: Run "desktop-rails init" first.'
    );
    process.exit(1);
  }
  ...
  const child = spawn("npx", ["tauri", "dev"], {
    ...
    env: {
      ...process.env,
      RUST_LOG: process.env.RUST_LOG || "desktop_rails=info",
    },
  });
...
function cmdBuild(args) {
  ...
  console.log(`Building Desktop Rails for ${target}...`);
...
Examples:
  desktop-rails new myapp                      # Create a new app from scratch
...
export function defaultUserAgent() {
  ...
  return `Desktop Rails/${packageVersion()} (${os}; ${arch})`;
}
...
export function desktopPackage(appName) {
  return {
    ...
    type: "module",
    scripts: {
      dev: "desktop-rails dev",
      build: "desktop-rails build",
      tauri: "tauri",
    },
    dependencies: {

...
#
# Configure how the Desktop Rails native shell presents your Rails routes.
...
DesktopRails.configure do |config|
  ...
  # Requests from the desktop app are marked with a Rails variant, so you can
  # write a whole template for it instead of branching inside a shared one:
  #
  #   app/views/orders/show.html.erb           # everyone
  #   app/views/orders/show.html+desktop.erb   # the desktop app
  #
  # Layouts work the same way (layouts/application.html+desktop.erb), and Rails
  # falls back to the plain template wherever no variant exists. Rename it, or
  # set it to nil to leave variants alone.

...
class PathsPlatformTest < Minitest::Test
  ...
  def test_macos_uses_application_support
    dir = DesktopRails::Paths.data_dir(
  ...
  def test_platform_detection
    assert_equal :macos, DesktopRails::Paths.platform("darwin24")
    assert_equal :windows, DesktopRails::Paths.platform("mingw32")
    assert_equal :windows, DesktopRails::Paths.platform("mswin64")
    assert_equal :linux, DesktopRails::Paths.platform("linux-gnu")
    # Anything unrecognised is treated as a unix, which is the useful guess.
    assert_equal :linux, DesktopRails::Paths.platform("freebsd14")
  end
  ...
  def test_returns_a_pathname_so_callers_can_join
    dir = DesktopRails::Paths.data_dir(app_id: "app", host_os: "darwin24", env: { "HOME" => "/h" })
  ...
  def test_does_not_create_the_directory_unless_asked
    Dir.mktmpdir do |tmp|
      ...
      DesktopRails::Paths.data_dir(env: { "DESKTOP_DATA_DIR" => target })
      refute File.directory?(target), "data_dir must not have side effects by default"

      DesktopRails::Paths.data_dir(env: { "DESKTOP_DATA_DIR" => target }, create: true)
...
class PathsSecretKeyBaseTest < Minitest::Test
  def test_generates_once_and_reuses_it
    Dir.mktmpdir do |tmp|
      ...
      first = DesktopRails::Paths.secret_key_base(env: env)
      second = DesktopRails::Paths.secret_key_base(env: env)
  ...
  def test_is_not_world_readable
    Dir.mktmpdir do |tmp|
      DesktopRails::Paths.secret_key_base(env: { "DESKTOP_DATA_DIR" => tmp })
  ...
  def test_environment_wins
    Dir.mktmpdir do |tmp|
      secret = DesktopRails::Paths.secret_key_base(
  ...
  def test_an_empty_file_is_regenerated
    ...
    Dir.mktmpdir do |tmp|
      ...
      secret = DesktopRails::Paths.secret_key_base(env: { "DESKTOP_DATA_DIR" => tmp })
...
class DesktopRailsAppIdentityTest < Minitest::Test
  ...
  def test_configuration_overrides_the_derived_identity
    ...
    assert_equal "dev.example.ledger", DesktopRails.app_id
    assert_equal "Ledger", DesktopRails.app_name
  end

  def test_data_dir_uses_the_app_id
    DesktopRails.configure { |config| config.app_id = "dev.example.ledger" }
    assert_includes DesktopRails.data_dir.to_s, "dev.example.ledger"
  end
end

...
- Packaging is Ruby too. `pack.sh`, `pack-linux.sh`, `pack-windows.ps1` and
  `vendor-path-gems.rb` are replaced by `DesktopRails::BundledPackage` on the
  `DesktopRails::Packager` layouts hosted mode already used, and
  `desktop-rails-tool package` runs it outside Rails. `desktop:package` builds
  in process instead of running a script, so it no longer looks for a checkout:
  `DESKTOP_RAILS_PACKAGING` and `config.packaging_dir` are gone, and an
  initializer that sets the latter must drop the line. The boot template,
  entitlements, launcher templates and the updater's `.mjs` files moved into the
  gem, so a gem installed from GitHub or RubyGems packages with nothing else.
  The Windows launcher takes its gem directory from the runtime's `rbconfig.rb`
  rather than naming `gems\3.4.0`, which would have dropped every default gem
  from `GEM_PATH` with a Ruby 4 runtime, and the shipped RubyInstaller no longer
  probes its own gem directory for writability on each launch. The macOS and
  Linux launchers name the ABI directory the same way. Linux tarballs keep the
  runtime's symbolic links as links.
...
- The Windows jobs of `fresh-app.yml` open the packaged window, for the fresh
...
- A packaged Windows app wrote into its own interpreter on every launch:
  RubyInstaller's `operating_system.rb` creates and deletes `writable_p` in the
  gem directory to choose `gem install` defaults. Packaging now replaces that
  probe in the interpreter it ships
  (`DesktopRails::Packager::WindowsGemProbe`) with the answer a read-only
  install gets.
- Every packaged Windows app opened a `cmd.exe` console window over its own
  ...
  prebuilt shell.
- The Windows launcher hardcoded Ruby 3.4's gem directory in `GEM_PATH`.
  Packaging now reads the ABI version from the runtime it ships, on every
  platform.
- The shell dropped the server's address when the server announced itself
  before the main window existed, so the window waited on the waiting page for
  the next connection probe, up to five seconds. Seen on Windows, where the
  window can take seconds to build. Needs a new shell release to reach apps
  that download the prebuilt shell.
- A Gemfile `ruby` pin to another patch release than the runtime's — what
  `rails new` writes for 7.0 and 7.1, a `.ruby-version`, a hand-written pin —
  made the runtime refuse the app: `desktop:gems` failed before installing, and a
  package built anyway died at launch with "Your Ruby version is 4.0.7, but your
  Gemfile specified 4.0.6". The staged Gemfile `desktop:gems` installs from and
  the packed copy of the app now ask for the runtime's Ruby, and Gemfile.lock
  records it; a requirement the runtime cannot meet stops packaging early with
  both versions named. The app's own Gemfile is never written.
